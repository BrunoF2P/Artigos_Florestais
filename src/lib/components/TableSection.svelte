    ...
    {#each pagedRows as article, idx (article.id ?? idx)}
      ...
      <div class="article-row">
        ...
        <span class="article-title-cell">
          ...
          <small>
            {#if article.document_type}{rv(article.document_type)}{:else}Sem
              tipo{/if}
        ...
        <span class="source-cell"
          >{trunc(article.source_title ?? article.source, 68)}</span
        >
        <span class="year-cell">{rv(article.ano)}</span>
        <span class="citation-cell"
          ><MessageCircle size={15} />{rv(article.cited_by ?? 0)}</span
        >
        ...
        {#each pagedRows as row, i (row.id ?? i)}
          <tr>
            <td class="id-cell">
              ...
              <span class="row-id">{rv(row.id)}</span>
            </td>
      ...
      <tbody>
        {#each pagedRows as row, i (row.id ?? i)}
          <tr>
            ...
            <td
              ><span
                class="type-pill {row.tipo === 'author' ? 'author' : 'index'}"
                >{rv(row.tipo)}</span
              ></td
            >
            <td>
        ...
        <button
          ...
          onclick={() =>
            dispatch("filterByEntity", {
              type: "referencia",
              id: Number(row.id),
              label: trunc(row.raw_reference, 48),
            })}
        >
          Ver artigos <ArrowRight size={14} />
        </button>
...
{#if rows.length > PAGE_SIZE}
  <div class="pagination-bar">
    <button type="button" disabled={currentPage === 0} onclick={() => goPage(0)}
      >Primeira</button
    >
    <button
      type="button"
      disabled={currentPage === 0}
      onclick={() => goPage(currentPage - 1)}>Anterior</button
    >
    <span>{currentPage + 1} de {totalPages}</span>
    <button
      type="button"
      disabled={currentPage >= totalPages - 1}
      onclick={() => goPage(currentPage + 1)}>Próxima</button
    >
    <button
      type="button"
      disabled={currentPage >= totalPages - 1}
      onclick={() => goPage(totalPages - 1)}>Última</button
    >
  </div>
{/if}
