...
export function inferTableType(fileName: string): string {
  const base = fileName.split(/[/\\]/).pop()?.replace(/\.csv$/i, '').toLowerCase().replace(/\s+/g, '_') ?? '';
...
export function looksLikeCombined(fields: string[], data: any[]): boolean {
  ...
  return low.includes('title') || low.includes('authors') || low.includes('author(s) id') || low.includes('author full names');
}
...
function hasReferenceEnd(value: string): boolean {
  const s = value.trim();
  return /\((18|19|20)\d{2}[a-z]?\)\s*$/i.test(s) || /,\s*(18|19|20)\d{2}\s*$/.test(s) || DOI_RE.test(s);
}
...
export function parseReference(raw: unknown): ParsedReference {
  ...
  const years = [...rawReference.matchAll(YEAR_RE)].map((m) => Number(m[0])).filter((y) => y >= 1800 && y <= new Date().getFullYear() + 1);
...
function extractReferenceTitle(raw: string, ano: number | null, doi: string | null): string | null {
  let text = raw;
  if (doi) text = text.replace(doi, '');
  if (ano) text = text.replace(new RegExp(`\\(?${ano}\\)?`, 'g'), '');
  text = text.replace(/\bdoi\s*:\s*/i, '').replace(/\s+/g, ' ').trim();
  ...
  const authorish = /(?:^|;\s*)[A-ZÀ-Ú][A-Za-zÀ-ÿ'’.-]+(?:\s+[A-Z]\.){0,4}\s*$/.test(beforeComma) || beforeComma.includes(';');
  ...
  const sourceIndex = parts.findIndex((part, index) => {
    ...
    return index > 0 && (/^\d+([-(]\d+[)]?)?$/.test(next) || /^pp\.?/i.test(next));
  });
  if (sourceIndex > 0) {
    titleParts = parts.slice(0, sourceIndex);
  }

  const title = titleParts.join(', ') || parts.find((p) => /[A-Za-zÀ-ÿ]{4,}/.test(p) && p.length >= 8) || parts[0] || null;
...
export function splitCombined(rows: any[]) {
  ...
  rows.forEach((row) => {
    ...
    if (isNaN(scopus_id)) scopus_id = Math.floor(Math.random() * 1000000000) + 9000000000;

    Artigo.push({
      scopus_id,
      titulo: cleanText(row['Title'] || row['title'] || row['Titulo']) || 'Sem título',
      ...
      linguagem: cleanText(row['Language of Original Document'] || row['Language']) || null,
    ...
    if (ids.length === fulls.length && ids.length) {
      ...
      parsedAuthors = fulls.map((f: string) => {
        const m = f.match(/^(.*)\s*\(([^)]+)\)\s*$/);
        return m ? { aid: m[2].trim(), name: m[1].trim() } : { aid: Math.floor(Math.random() * 100000000).toString(), name: f.trim() };
      });
    } else {
      parsedAuthors = auth.map((a: string) => ({ aid: Math.floor(Math.random() * 100000000).toString(), name: a.trim() }));
    }

    parsedAuthors.forEach(({ aid, name }) => {
      const short = name.split(',')[0] || name.split(' ')[0] || name;
      let numericAid = parseInt(aid.replace(/[^0-9]/g, ''), 10);
      if (isNaN(numericAid)) numericAid = Math.floor(Math.random() * 1000000000) + 9000000000;

      if (!Autor[numericAid]) {
        Autor[numericAid] = { id: numericAid, nome: short, nome_completo: name };
      }
      Artigo_autor.push({ temp_scopus_id: scopus_id, autor_id: numericAid, _temp_titulo: row['Title'] });
    });
    ...
    const addKeywords = (key: string, tipo: string) => {
      splitList(row[key]).forEach((k: string) => {
        ...
        Artigo_PalavraChave.push({ temp_scopus_id: scopus_id, temp_kw_id: Palavras_chaves[kwKey].temp_id, _temp_titulo: row['Title'] });
      });
    };
    ...
    splitScopusReferences(row['References']).forEach((rawReference: string) => {
      ...
      Artigo_referencia.push({ temp_scopus_id: scopus_id, temp_ref_id: Referencia[parsed.raw_reference].temp_id, _temp_titulo: row['Title'] });
    });
    ...
    normalizeOpenAccess(row['Open Access']).forEach((name: string) => {
      ...
      Artigo_OpenAccess.push({ temp_scopus_id: scopus_id, temp_oa_id: OpenAccessTipo[key].temp_id, _temp_titulo: row['Title'] });
    });
  });
