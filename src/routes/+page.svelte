<script lang="ts">
  ...
  type ActiveFilter = {
    type: "autor" | "keyword" | "referencia" | "open_access";
    id: number;
    label: string;
  } | null;
  ...
  async function loadAllChartArticles(): Promise<
    { ano: unknown; cited_by: unknown }[]
  > {
    const pageSize = 1000;
    const allRows: { ano: unknown; cited_by: unknown }[] = [];
    let offset = 0;
  ...
  async function upsertPalavrasChave(
    ...
    for (const chunk of chunkArray(keywords, 300)) {
      const rows = chunk.map((kw: any) => ({
        palavra: kw.palavra,
        tipo: kw.tipo,
      }));
      const { error } = await supabase.from("palavra_chave").insert(rows);
      if (error && error.code !== "23505")
        console.error("Erro inserir palavras-chave:", error);
    }
    ...
    for (const chunk of chunkArray(allPalavras, 200)) {
      const { data } = await supabase
        ...
        .in("palavra", chunk);
      data?.forEach((e: any) =>
        kwMap.set(e.palavra.toLowerCase() + "_" + e.tipo, Number(e.id)),
      );
    }
    return kwMap;
  }
  ...
  async function upsertReferencias(refs: any[]): Promise<Map<string, number>> {
    ...
    for (const chunk of chunkArray(uniqueRefs, 500)) {
      ...
      const { error } = await supabase
        .from("referencia")
        .upsert(rows, { onConflict: "raw_reference" });
      ...
      data?.forEach((e: any) => {
        if (e.raw_reference)
          refMap.set(String(e.raw_reference).trim(), Number(e.id));
      });
    }
    return refMap;
  }
  ...
  async function insertInChunks(
    ...
    for (const chunk of chunkArray(rows, chunkSize)) {
      ...
      if (error && error.code !== "23505")
        console.error(`Erro inserir em ${table}:`, error);
    }
  }
  ...
  async function importCsvFiles(files: File[]) {
    ...
    try {
      ...
      for (const r of PARSED["artigo_palavra_chave"] || []) {
        const aid = articleIdByScopusId.get(Number(r.temp_scopus_id));
        const kwObj = (PARSED["palavra_chave"] || []).find(
          (k: any) => k.temp_id === r.temp_kw_id,
        );
      ...
      for (const r of PARSED["artigo_referencia"] || []) {
        const aid = articleIdByScopusId.get(Number(r.temp_scopus_id));
        const refObj = (PARSED["referencia"] || []).find(
          (ref: any) => ref.temp_id === r.temp_ref_id,
        );
        const refRealId = refObj
          ? refMap.get(String(refObj.raw_reference ?? "").trim())
          : undefined;
        if (aid && refRealId)
      ...
      for (const r of PARSED["artigo_open_access"] || []) {
        const aid = articleIdByScopusId.get(Number(r.temp_scopus_id));
        const oaObj = (PARSED["open_access_tipo"] || []).find(
          (oa: any) => oa.temp_id === r.temp_oa_id,
        );
